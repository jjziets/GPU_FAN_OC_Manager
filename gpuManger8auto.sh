...
mapFanIndex()
{
	echo "Start Mapping gpu to fan"
	xinit  ${SET}   -a [gpu:0]/GPUFanControlState=1 -a [gpu:0]/GPUPowerMizerMode=1 -a [fan:0]/GPUTargetFanSpeed=100 \
        	        -a [gpu:1]/GPUFanControlState=1 -a [gpu:1]/GPUPowerMizerMode=1 -a [fan:1]/GPUTargetFanSpeed=100 \
                        -a [gpu:2]/GPUFanControlState=1 -a [gpu:2]/GPUPowerMizerMode=1 -a [fan:2]/GPUTargetFanSpeed=100 \
                        -a [gpu:3]/GPUFanControlState=1 -a [gpu:3]/GPUPowerMizerMode=1 -a [fan:3]/GPUTargetFanSpeed=100 \
                        -a [gpu:4]/GPUFanControlState=1 -a [gpu:4]/GPUPowerMizerMode=1 -a [fan:4]/GPUTargetFanSpeed=100 \
                        -a [gpu:5]/GPUFanControlState=1 -a [gpu:5]/GPUPowerMizerMode=1 -a [fan:5]/GPUTargetFanSpeed=100 \
               		-a [gpu:6]/GPUFanControlState=1 -a [gpu:6]/GPUPowerMizerMode=1 -a [fan:6]/GPUTargetFanSpeed=100 \
                	-a [gpu:7]/GPUFanControlState=1 -a [gpu:7]/GPUPowerMizerMode=1 -a [fan:7]/GPUTargetFanSpeed=100 --  :0 -once

	for n in  $(seq 0 $((numGPUs-1))); do
		xinit  ${SET} -a [fan:$n]/GPUTargetFanSpeed=50 --  :0 -once
		sleep 10
...
#        [ $speed -lt $((currentSpeed[i]-1)) -o $speed -gt $((currentSpeed[i]+1)) ] && Fspeed[$i]=$speed
 	...
 	for i in $(seq 0 $((numGPUs-1))); do
        if [[ "${currentSpeed[$i]}" != "${Fspeed[$i]}" ]]; then
			xinit ${SET}    -a [gpu:$i]/GPUFanControlState=1    -a [fan:$i]/GPUTargetFanSpeed=${Fspeed[${gpuFanIndex[$i]}]} --  :0 -once
			echo "GPU0$i current ${currentSpeed[$i]} and new speed ${Fspeed[$i]}"
	fi
	done
